...
def pkgException(pkg_name, level):
    ...
    meta = pkg_meta_template(level=level)
    meta["pkg"]["pkgName"] = pkg_name
...
def req_pypi(pkg_name, level):
    ...
    if not meta:
        meta = pkgException(pkg_name, level)
    return meta
...
def makeBOM(bomInfo, filepath = "-", fileformat = "txt", model = "ossbom"):
    ...
    if model == "ossbom":
        ...
        for ctinfo in bomDict.get("ComponentInformation", []):
            info = {}
            info.update(ctinfo)
            for valinfo in bomDict["ValidityInformation"]:
        ...
        if model == "spdx":
            ...
            for ID, info in all_pkg_info.items():
                ...
                cdx_component = cyclonedx.model.component.Component(
                    ...
                    licenses=[lc_factory.make_with_expression(info["DeclaredLicense"])] if info.get("DeclaredLicense", None) else None,
